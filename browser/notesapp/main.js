...
function renderNotes() {
    ...
    container.innerHTML = notes.map(note => `<div id="${'noteTemplate' + note.key}" class="flex justify-between items-center px-[2px] border-b border-black">
            <div id="${'noteInfo' + note.key}" class="flex items-center">
                ...
                <label id="${'noteTxt + note.key'}" for="${'noteCheck' + note.key}" class="w-[200px] max-h-[100px] overflow-auto break-words whitespace-normal text-base ${note.softDeleted ? 'line-through text-gray-500' : ''}">${note.note}</label>
            </div>
